...
//        webContext.setDescriptor("/Users/jiangshan/develop/workspace/sxyx-web-sample/src/main/webapp/WEB-INF/web.xml");
    ...
    public static void startJetty(int port, String contextPath, String webappRootPath, String webappPath) {
        final Server server = Main.createServerInSource(port, contextPath, webappRootPath, webappPath);
        try {
            server.stop();
            server.start();
            server.join();
    ...
    public static void main(String[] args) {
        String webappRootPath = Main.class.getClassLoader().getResource(".").getPath();
        System.setProperty("webappRootPath", webappRootPath);
        InputStream inputStream = null;
        try {
            inputStream = Main.class.getClassLoader().getResource("run.properties").openStream();
            Properties properties = new Properties();
            properties.load(inputStream);
            int port = Integer.valueOf(properties.getProperty("jetty.port"));
            String context = properties.getProperty("jetty.context");
